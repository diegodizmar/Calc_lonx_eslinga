...
def calcular_op_z(orelleta_z, orelleta_x, orelleta_y, CdG_X, CdG_Y, Lonxitude_min_Eslinga):
    ...
    if discriminant < 0:
        st.error("Discriminante negativo, no hay solución real.")
        return None
...
def main():
    ...
    st.header("Ingrese las coordenadas de las Orelletas y el centro de gravedad")
    ...
    with st.expander("Centro de Gravedad"):
        col1, col2, col3 = st.columns(3)
        with col1:
            st.write("X")
    ...
    # Separación y campo de longitud mínima de eslinga
    st.write(" ")
    Lonxitude_min_Eslinga = st.number_input("Longitud mínima de Eslinga", value=0.0, format="%.2f")
    ...
    if st.button("Calcular"):
        try:
            ...
            # Calcular el mínimo de los primeros elementos de cada lista
            min_01 = min(OL_1[0], OL_2[0], OL_3[0], OL_4[0])

            # Determinar coord_o basado en el valor mínimo
            if min_01 == OL_1[0]:
            ...
            # Mostrar resultados de distancias con etiquetas en negrita si cumplen la condición
